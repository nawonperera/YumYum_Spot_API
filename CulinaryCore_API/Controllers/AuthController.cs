...
public class AuthController : Controller
{
    ...
    private readonly UserManager<ApplicationUser> _userManager; // This is a service provided by Identity. // It is used to create, update, delete, and manage users. // Example: creating a user, checking password, adding roles, etc.
    ...
    [HttpPost("register")] //“When someone sends a POST request to /register, run this method”
    public async Task<IActionResult> Register([FromBody] RegisterRequestDTO model)
    {
        if (ModelState.IsValid)
        {
            ...
            if (result.Succeeded)
            {
                ...
                // GetAwaiter().GetResult() is used here to synchronously wait for the result in a async method
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequestDTO model)
    {
        if (ModelState.IsValid)
        {
            ...
            if (userFromDb != null)
            {
                ...
                byte[] key = System.Text.Encoding.ASCII.GetBytes(_secretKey); // Convert the secret key (string) into a byte array. This key is used to sign the JWT token. Describe the details of the JWT token we want to create
                ...
                SecurityTokenDescriptor tokenDescriptor = new()
                {
                    ...
                    Subject = new ClaimsIdentity(
                        ...
                        new Claim(ClaimTypes.Email, userFromDb.Email!.ToString()), // Standard claim: user's email address. !. means, we are sure Email is not null
                        new Claim(ClaimTypes.Role, _userManager.GetRolesAsync(userFromDb).Result.FirstOrDefault()!) // Add the user's role as a claim in the JWT token. ClaimTypes.Role → standard claim name for user roles. _userManager.GetRolesAsync(userFromDb) → gets all roles assigned to the user. .Result → waits for the result synchronously (not recommended, but works). .FirstOrDefault() → takes the first role from the list (e.g., "Admin" or "Customer"). ! → tells the compiler that the value will not be null
                        ]),
